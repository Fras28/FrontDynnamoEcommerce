...
import { PackagePlus, Edit3, Save, X, Loader2, Tag } from 'lucide-react';
...
const ProductForm = ({ editingProduct, onCancel }: ProductFormProps) => {
  ...
  const onSubmit = async (data: ProductFormData) => {
    // Convertir categoryId vacío a undefined
    const submitData = {
      ...data,
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
        ...
        <div className="space-y-1">
          <label className="text-[10px] font-black text-slate-500 uppercase ml-2 tracking-widest flex items-center gap-2">
            <Tag size={12} /> Categoría (Opcional)
          </label>
          <select
            {...register('categoryId', { 
              setValueAs: (v) => v === '' ? undefined : Number(v) 
            })}
            className="w-full bg-slate-950 border border-slate-800 rounded-xl p-4 text-sm outline-none focus:ring-2 focus:ring-indigo-500 text-black cursor-pointer"
          ...
          {loadingCategories && (
            <p className="text-slate-500 text-xs mt-1 ml-2 flex items-center gap-2">
              <Loader2 size={12} className="animate-spin" /> Cargando categorías...
            </p>
          )}
          {errors.categoryId && (
            <p className="text-red-400 text-xs mt-1 ml-2">{errors.categoryId.message}</p>
          )}
        </div>
