...
const ProductDetail = () => {
  ...
  useEffect(() => {
    ...
    const checkFavorite = async () => {
      try {
        ...
        setIsFavorite(data.some((fav: any) => fav.productId === productId));
      } catch {
        setIsFavorite(false);
      }
    };
  ...
  const handleToggleFavorite = async () => {
    ...
    try {
      ...
      notifications.show({ title: 'Error', message: 'No se pudo actualizar favoritos', color: 'red' });
    } finally {
      setIsToggling(false);
    }
  };
  ...
  if (!productId) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center p-4 text-center">
        ...
        <button onClick={() => navigate('/catalogo')} className="bg-indigo-600 hover:bg-indigo-500 text-white px-6 py-3 rounded-xl font-bold transition-all">
          Volver al Catálogo
        </button>
      </div>
    );
  ...
  const handleAddToCart = () => {
    if (product.hasVariants) {
      const selectedVariant = product.variants?.find(
        (v) => v.sizeId === selectedSize && v.colorId === selectedColor && v.isActive,
      );
      if (!selectedVariant) {
        notifications.show({ title: 'Selección requerida', message: 'Por favor selecciona un talle y/o color', color: 'orange' });
        return;
      }
      if (selectedVariant.stock === 0) {
        notifications.show({ title: 'Sin stock', message: 'Esta variante no tiene stock disponible', color: 'red' });
        return;
      }
      addToCart({ ...product, selectedVariantId: selectedVariant.id, selectedVariant }, 1);
    } else {
      addToCart(product, quantity);
    }
    notifications.show({ title: 'Producto agregado', message: `"${product.name}" se agregó al carrito`, color: 'green' });
  };
  ...
  // Variante actualmente seleccionada (para mostrar stock puntual)
  const selectedVariant = product.variants?.find(
    (v) => v.sizeId === selectedSize && v.colorId === selectedColor && v.isActive,
  );
...
/>
      ...
      <div className="w-full max-w-7xl mx-auto px-3 sm:px-4 grid lg:grid-cols-2 gap-6 sm:gap-8 lg:gap-12 mt-2 sm:mt-4">
        ...
        <div className="space-y-3 sm:space-y-4 w-full">
          ...
          {/* Miniaturas */}
          {images.length > 1 && (
            <div className="flex gap-2 sm:gap-3 overflow-x-auto pb-2 -mx-3 px-3 sm:mx-0 sm:px-0" style={{ scrollbarWidth: 'none' }}>
              {images.map((url, idx) => (
                <button
                  key={idx}
        ...
        {/* ── Panel de info ────────────────────────────────────────── */}
        <div className="bg-slate-900/50 border border-slate-800 rounded-2xl sm:rounded-[2rem] p-4 sm:p-6 lg:p-8 space-y-5 w-full">
          ...
          <div>
            ...
            <h1 className="text-2xl sm:text-3xl lg:text-4xl font-black mb-3 leading-tight break-words">
              {product.name}
            </h1>
          ...
          {product.hasVariants && product.variants && product.variants.length > 0 && (
            <div className="space-y-4 pt-2 border-t border-slate-800">
              ...
              {sizes && sizes.length > 0 && (
                <div>
                  <label className="text-xs font-black text-slate-400 uppercase mb-2 block">Talle</label>
                  <div className="flex gap-2 flex-wrap">
                    {sizes.map((size) => {
                      const variantsWithSize = product.variants?.filter(
                        (v) => v.sizeId === size.id && v.isActive,
                      );
              ...
              {colors && colors.length > 0 && (
                <div>
                  <label className="text-xs font-black text-slate-400 uppercase mb-2 block">Color</label>
                  <div className="flex gap-2 flex-wrap">
                    {colors.map((color) => {
                      const variantsWithColor = product.variants?.filter(
                        (v) => v.colorId === color.id && v.isActive,
                      );
              ...
              {selectedVariant && (
                <div className="bg-slate-900/50 border border-slate-800 rounded-xl p-3">
                  <p className="text-xs text-slate-400">
                    Stock disponible:{' '}
                    <span className={`font-bold ${selectedVariant.stock <= 5 ? 'text-amber-400' : 'text-emerald-400'}`}>
          ...
          <div className="bg-slate-900/50 border border-slate-800 rounded-2xl p-4 sm:p-5 space-y-4">
            {/* Selector de cantidad (solo sin variantes) */}
            {!product.hasVariants && (
              <div className="flex items-center justify-between gap-4">
                <span className="font-bold text-[10px] sm:text-sm tracking-widest text-slate-400 whitespace-nowrap">
                  CANTIDAD
                </span>
            ...
            <button
              ...
              className="w-full bg-indigo-600 text-white py-4 sm:py-5 rounded-xl font-black text-xs sm:text-sm flex items-center justify-center gap-2 sm:gap-3 hover:bg-indigo-500 transition-all disabled:bg-slate-800 disabled:text-slate-500 group"
            >
              {isOutOfStock ? (
                <>
                  <Package size={18} />
            ...
            <div className="grid grid-cols-1 gap-3 pt-3 border-t border-slate-800">
              ...
              <div className="flex items-center gap-2 sm:gap-3 text-slate-400 justify-center sm:justify-start">
                <RotateCcw size={16} className="text-indigo-500 flex-shrink-0" />
                <span className="text-[9px] sm:text-[10px] font-bold whitespace-nowrap">
                  DEVOLUCIÓN 30 DÍAS (producto cerrado)
                </span>
              </div>
            </div>
          </div>
