    ...
    <div className={`relative overflow-hidden ${aspectRatioClasses[aspectRatio]} ${className}`}>
      ...
      {showPlaceholder && imageState === 'loading' && (
        <div className="absolute inset-0 bg-slate-800 animate-pulse flex items-center justify-center">
          <div className="w-8 h-8 border-4 border-slate-700 border-t-slate-600 rounded-full animate-spin" />
        </div>
      )}
      ...
      {imageState === 'error' && (
        <div className="absolute inset-0 bg-slate-800 flex items-center justify-center border border-slate-700">
          <div className="text-center">
            <ImageOff size={32} className="text-slate-600 mx-auto mb-2" />
            <p className="text-slate-600 text-xs font-bold">Imagen no disponible</p>
          </div>
        </div>
      )}
