...
const UserOrders = () => {
  const { data: orders, isLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      console.log('📦 ORDERS DATA:', data); // Debug: ver toda la data
      return data;
    },
  });
    ...
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      {orders?.length === 0 ? (
        ...
        orders?.map((order: any) => {
          console.log('🔍 ORDER:', order); // Debug: ver cada orden
              ...
              <div className="p-6 space-y-4">
                ...
                <div className="space-y-2">
                  {order.items.map((item: any) => {
                    console.log('🎁 ITEM:', item); // Debug: ver cada item
                    console.log('📸 PRODUCT:', item.product); // Debug: ver el producto
                    console.log('🖼️ IMAGES:', item.product?.images); // Debug: ver las imágenes
                    
                    // ✅ Múltiples estrategias para obtener la imagen
                    let productImage = 'https://placehold.co/100x100/1e293b/4f46e5?text=Sin+Imagen';
                    
                    // Estrategia 1: Array de imágenes (nueva estructura)
                    if (item.product?.images && Array.isArray(item.product.images) && item.product.images.length > 0) {
                      productImage = item.product.images[0].url;
                      console.log('✅ Imagen desde array:', productImage);
                    }
                    // Estrategia 2: imageUrl directo (estructura antigua - fallback)
                    else if (item.product?.imageUrl) {
                      productImage = item.product.imageUrl;
                      console.log('✅ Imagen desde imageUrl:', productImage);
                    }
                    // Estrategia 3: Placeholder
                    else {
                      console.log('⚠️ No hay imagen disponible, usando placeholder');
                    }
                    ...
                    return (
                      <div key={item.id} className="flex items-center gap-3 text-sm">
                        ...
                        <div className="flex-1">
                          <p className="text-slate-200 font-medium leading-none">{item.product?.name || 'Producto sin nombre'}</p>
